...
export const mlog =(...arg)=>{
  //const M_DEBUG = import.meta.env.M_DEBUG
  // if(['error','log'].indexOf( arg[0] )>-1 ){ //必须显示的
  // }else  if(! isNotEmptyString(import.meta.env.M_DEBUG) ) return ;
...
export const auth = async ( req :Request , res:Response , next:NextFunction ) => {
  ...
  if (isNotEmptyString(AUTH_SECRET_KEY)) {
    try {
      ...
      //const auth_secret_keys = import.meta.env.AUTH_SECRET_KEY.trim().split(',').filter(item => item !== '');
      const auth_secret_keys = import.meta.env.AUTH_SECRET_KEY? import.meta.env.AUTH_SECRET_KEY.trim().split(',').filter(item => item !== ''):[];
...
const checkLimit=  ( req :Request , res:Response )=>{
  if ( !isNotEmptyString( import.meta.env.AUTH_SECRET_ERROR_COUNT )) {
    return ;
  }
  
  const bTime = import.meta.env.AUTH_SECRET_ERROR_TIME??10;
  // 允许的最大错误次数
  const maxErrorCount =  +import.meta.env.AUTH_SECRET_ERROR_COUNT;
  // 禁止登录的时间（毫秒）
...
export const turnstileCheck= async ( req :Request , res:Response , next:NextFunction ) => {
   ...
   const TURNSTILE_SITE = import.meta.env.TURNSTILE_SITE
    if (!isNotEmptyString(TURNSTILE_SITE)) {
      next();
      return ;
    }
    //TURNSTILE_NO_CHECK
    if ( isNotEmptyString( import.meta.env.TURNSTILE_NO_CHECK)) { //前端显示当时后端不check
       next();
       return ;
    }
    try{
...
const getCookie=( time:string )=>{
  return time+'_'+(md5(time + import.meta.env.TURNSTILE_SECRET_KEY ).substring(0,10) );  
}
