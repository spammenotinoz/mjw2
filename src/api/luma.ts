...
// Initialize LUMA_KEY and LUMA_SERVER from environment variables
function initializeLumaConfig() {
  const lumaKey = import.meta.env.VITE_LUMA_KEY;
  const lumaServer = import.meta.env.VITE_LUMA_SERVER;
  
  //if (lumaKey && lumaServer) {
    gptServerStore.myData.LUMA_KEY = lumaKey;
    gptServerStore.myData.LUMA_SERVER = lumaServer;
