...
export const  getUrl=(url:string)=>{
    ...
    // If server URL is provided and contains /runwayml or /pro, use as-is
    if(server && (server.indexOf('/runwayml')>0 || server.indexOf('/pro')>0))
        return `${server}${pro_prefix}/runwayml${url}`;
    // If server URL is provided, add /runwayml prefix
    if(server)
        return `${server}${pro_prefix}/runwayml${url}`;
    // Otherwise use local backend proxy with /runwayml prefix
